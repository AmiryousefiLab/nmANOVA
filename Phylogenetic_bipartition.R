#Codes for phylogenetic bipartition example (Figure4 A) from the manuscript 
...
df <- read_excel( "phylogenetic_bipartition_data.xlsx" )
rownames( df ) <- colnames( df ) 

partition <- sub('_[^_]*$', '', colnames( df  ) ) 
res <- nmANOVA( df , partition, N_sampling = 10 ) 
res$summary$p_value # 0.058

df <- as.matrix( df )

res_hdbscan <- hdbscan( df, minPts = 5 )
plot( res_hdbscan, show_flat = TRUE )


partition <- res_hdbscan$cluster
res_nmA_hdbscan <- nmANOVA( df, partition, N_sampling = 10 ) 
res_nmA_hdbscan$summary$p_value # 0.058

mds_matrix <- matrix( NA, ncol = ncol(df), nrow = nrow(df) )
mds_matrix[upper.tri(mds_matrix)] <- rowMeans(cbind(df[upper.tri(df, diag = FALSE)], 
                                    df[lower.tri(df, diag = FALSE)]))
mds_matrix[lower.tri(mds_matrix)] <- rowMeans(cbind(df[upper.tri(df, diag = FALSE)], 
                                    df[lower.tri(df, diag = FALSE)]))
diag(mds_matrix ) <- 0 #diag( df)
colnames( mds_matrix )<- colnames(df)
rownames( mds_matrix )<- rownames(df)


mds.coor=cmdscale( as.dist( ( df ) ) )
mds.coor <- as.data.frame( mds.coor)
colnames( mds.coor ) <- c( 'coor1', 'coor2' )
mds.coor <- 
  mds.coor %>%
  mutate( Species =  sub('_[^_]*$', '', rownames( mds.coor ) ), label = rownames( mds.coor) )
...
ggplot( mds.coor, aes( x =  ( coor1 ), y = ( coor2 ), shape = as.factor( Species ), col = as.factor( Species ) ) ) +  
  ...
  scale_y_continuous(labels = scientific_10) + scale_x_continuous(labels = scientific_10) +
  annotate("text", x=0.0010, y=0.0033, label= expression( italic( 'p-value = 0.044' ) ), size = 10) + 
  annotate("text", x=-0.0010, y=-0.0033, label= expression(  italic( 'p-value = 0.058'  ) ), size = 10) 
